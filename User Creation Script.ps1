...
New-ADUser -Name "$Lastname$Initial" -GivenName "$User" -Surname "$Lastname" -SamAccountName "$Lastname$Initial" -UserPrincipalName "$Initial.$Lastname@$Domain" -AccountPassword (ConvertTo-SecureString -AsPlainText "$Password" -Force) -Enabled $true -ChangePasswordAtLogon $true -path $OU
